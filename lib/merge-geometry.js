...
import clone from 'clone-geometry'

export default function merge(geometry1, geometry2) {
    if (geometry1.geometry) return mergeMesh(geometry1, geometry2)
    return mergeGeometry(geometry1, geometry2)
}

function mergeGeometry(geometry1, geometry2) {
...
function mergeMesh(mesh1, mesh2) {
    let geometries = [mesh1, mesh2].map(mesh => {
        let geometry = clone(mesh.geometry)
